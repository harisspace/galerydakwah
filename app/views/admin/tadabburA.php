...
<div class="modal">
    <form action="<?= BASEURL; ?>/tadabbur/tambah" method="POST">
...
<div class="track">
    <?php foreach ($data['videos'] as $video) : ?>
        <a href="<?= BASEURL; ?>/tadabbur/video/<?= $video['id'] ?>" class="track-video is-black"><?= $video["judul"] ?>
            <a href="<?= BASEURL; ?>/tadabbur/hapus/<?= $video['id'] ?>" class="is-black link" onclick="return confirm('apakah anda yakin??')">hapus</a> |
            <a href="<?= BASEURL; ?>/tadabbur/ubah/<?= $video['id'] ?>" class="is-black link">ubah</a>
        </a>
    <?php endforeach; ?>
</div>