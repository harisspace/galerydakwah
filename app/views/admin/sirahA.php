...
<div class="modal">
    <form action="<?= BASEURL; ?>/sirah/tambah" method="POST">
...
<div class="track">
    <?php foreach ($data['videos'] as $video) : ?>
        <a href="<?= BASEURL; ?>/sirah/video/<?= $video['id'] ?>" class="track-video is-black"><?= $video["judul"] ?>
            <a href="<?= BASEURL; ?>/sirah/hapus/<?= $video['id'] ?>" class="is-black link" onclick="return confirm('apakah anda yakin??')">hapus</a> |
            <a href="<?= BASEURL; ?>/sirah/ubah/<?= $video['id'] ?>" class="is-black link">ubah</a>
        </a>
    <?php endforeach; ?>
</div>